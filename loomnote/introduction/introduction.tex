...
\section{loomnote模板介绍}

\textcolor{indigo}{本 \textsc{loomnote}} 模板基于 \textcolor{madder}{北极甜虾} 的 \textcolor{weld}{\text{LOOM} template} 织就，该作者已在 \textcolor{madder}{GitHub} 开源 \textcolor{selvage}{(Polaris-Aeterna/loom-notes)}。

loomnote 基本保留了 \textcolor{weld}{\text{LOOM} template} 的样式和环境定义，但作了以下适配 \textcolor{selvage}{Overleaf} 的改动。
...
原模板依赖 macOS 特有商业字体（Optima、Avenir Next），未安装时编译报错。本模板改用 URW Classico（Optima 的开源复刻），用户需自行将字体文件上传至项目 \texttt{fonts/} 目录。
